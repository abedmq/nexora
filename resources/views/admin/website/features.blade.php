...
@section('breadcrumb')<span class="separator"><i class="fas fa-chevron-left"></i></span><span>ثوابت الموقع</span><span class="separator"><i class="fas fa-chevron-left"></i></span><span>المميزات</span>@endsection

@section('content')
<div class="d-flex justify-content-between align-items-center mb-3">
    <p class="text-muted mb-0" style="font-size:13px;">العناصر تظهر في قسم "المميزات" بالصفحة الرئيسية</p>
    <button class="btn btn-nx-primary btn-sm" data-bs-toggle="modal" data-bs-target="#addModal"><i class="fas fa-plus me-1"></i> إضافة ميزة</button>
</div>
<div class="nx-card">
            ...
            <tbody>
                ...
                <tr class="{{ !$item->is_active ? 'opacity-50' : '' }}">
                    ...
                    <td>
                        ...
                        <form action="{{ route('admin.website.features.destroy', $item->id) }}" method="POST" class="d-inline del-form">@csrf @method('DELETE')<button class="btn btn-sm btn-nx-secondary text-danger"><i class="fas fa-trash"></i></button></form>
                    </td>
                </tr>
                @empty
...
<div class="modal fade" id="addModal" tabindex="-1"><div class="modal-dialog"><div class="modal-content"><form action="{{ route('admin.website.features.store') }}" method="POST">@csrf
<div class="modal-header"><h5 class="modal-title">إضافة ميزة</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
<div class="modal-body">
    <div class="mb-3"><label class="form-label">الأيقونة (Font Awesome)</label><input type="text" name="icon" class="form-control" value="fas fa-star" dir="ltr"></div>
...
<div class="modal-header"><h5 class="modal-title">تعديل الميزة</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
<div class="modal-body">
...
@section('scripts')
<script>
$(document).on('click','.edit-btn',function(){var d=$(this).data();$('#editForm').attr('action','/admin/website/features/'+d.id);$('#eIcon').val(d.icon);$('#eTitle').val(d.title);$('#eDesc').val(d.description);new bootstrap.Modal('#editModal').show();});
